...
import PetList from '../components/container/PetList'; // Asegúrate de que ahora se llama PetList
import { Center } from '../interfaces/Center';
import { Pet } from '../interfaces/Pet';
import { fetchCenterDetail, fetchPets } from '../api/centerDetail';
...
const CenterDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCenterData = async () => {
      try {
        const centerData = await fetchCenterDetail(id!);
  ...
  return (
    <div className="w-full p-6 bg-gradient-to-r from-green-50 to-blue-50 shadow-lg rounded-lg m-4">
      ...
      <div className="mt-6">
        ...
        <PetList pets={pets} /> {/* Cambiado a PetList y props 'pets' */}
      </div>
    </div>
  );
};
